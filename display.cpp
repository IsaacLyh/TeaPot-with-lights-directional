...
void display() 
{
  ...
  if (numused) {
    ...
    // Remember that light positions must be transformed by modelview.
          for (int it = 0; it < numLights; it++) {
              ...
              transformvec(tmp,out);
              for (int i = 0; i < 4; i++) {
                  lightransf[offset + i] = out[i];
  ...
  for (int i = 0 ; i < numobjects ; i++) {
    ...
    // Again glUniform() related functions will be useful
      ...
      //set transformation for each obj;
      modelview = transf * obj->transform;
      GLfloat ambientM[4] = {obj->ambient[0], obj->ambient[1], obj->ambient[2], obj->ambient[3]};
      GLfloat diffuseM[4] = {obj->diffuse[0], obj->diffuse[1], obj->diffuse[2], obj->diffuse[3]};
      GLfloat specularM[4] = {obj->specular[0], obj->specular[1], obj->specular[2], obj->specular[3]};
      GLfloat emissionM[4] = {obj->emission[0], obj->emission[1], obj->emission[2], obj->emission[3]};
      
      glUniform4fv(ambientcol, 1, ambientM);
      glUniform4fv(diffusecol, 1, diffuseM);
      glUniform4fv(specularcol, 1, specularM);
      glUniform4fv(emissioncol, 1, emissionM);
